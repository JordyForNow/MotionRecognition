...
namespace Sandbox
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            DirectoryInfo d = new DirectoryInfo(@"./Data_Predict/good");
            ...
            foreach (var file in d.GetFiles("*.csv"))
            {
                ...
                predictSettings.predictData = file.FullName;


                CountNetworkPredictController.PreparePredictor(ref container, ref predictSettings);

				bool result = CountNetworkPredictController.Predict(ref container, ref predictSettings);
			...
			Console.WriteLine("Good movement files: " + goodCounter);
			Console.WriteLine("Bad movement files: " + badCounter);
        }
    }
}
